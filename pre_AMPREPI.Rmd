...
fnFs <- sort(intersect(list.files(rawDir, pattern="_R1", full.names = TRUE), list.files(rawDir, pattern="fastq", full.names = TRUE)))

samples.forward <- sapply(strsplit(basename(fnFs), "_"), `[`, 1)

## Reverse files
fnRs <- sort(intersect(list.files(rawDir, pattern="_R2", full.names = TRUE), list.files(rawDir, pattern="fastq", full.names = TRUE)))
