...
if (dir.exists(file.path(reportDir, "cutadapted"))) {
  unlink(file.path(reportDir, "cutadapted"), recursive = TRUE)
}
...
```{r typingControl, eval = firstRun2}
...
```{r decipher, eval = FALSE, include=FALSE}
...
``` {r dada2Typing, eval = FALSE, include = FALSE}
