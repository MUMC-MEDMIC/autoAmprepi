...
All the merged reads were subject of classification using using `r typeTyping`.
There are `r taxFlt` taxa in `r nsampFlt` samples.
The taxa abundance was calculated by the sum per-taxon.
...
For individual sample, each ASV’s counts were divided by the total number of counts to get proportion.
Each dot presents an ASV at the Phylum level.
Prevalence shows how often such ASV appears in all samples.
Total abundance displays the number of reads per ASV.    
...
The raw phyloseq file can be downloaded: `r xfun::embed_file(file.path(reportDir,"ps_no_Chime.RDS"), text = "Here!")`
...
if ("Sample_Type" %in% colnames(filterdExp)) {
  ...
  if ("Negative" %in% filterdExp$Sample_Type) {
...
Tracking reads through different stages in the pipeline. If majority of reads were removed then it is better to revisit settings from the pipeline.  


```{r finalReads, message=FALSE, warning=FALSE}
...
if (file.exists(file.path(reportDir,"reads_tracking.csv"))) {
  ...
  if (contamRemov) {
    ...
    meltTrack <- reshape2::melt(track, id.var= "Samples", value.name = "Reads", variable.name="Stage")
    ggplot(data = meltTrack, aes(x=Stage, y=Reads, group=Samples, color=Samples)) + geom_line() + geom_point() + scale_y_continuous(expand = c(0.05,0), limits = c(0, NA)) + coord_cartesian(clip='off')
...
Mocks and Negative controls were removed from the final phyloseq file. The final phylum composition is shown below.


```{r phyFinFeatures, eval = mockAnalysis}

## If found sample then remove Mocks
if (any(sample_data(fltps.ContamFree)$Sample_Type == "Sample")) {
...
## If no Mocks in the original experiment
finPs <- fltps.ContamFree
```
